...
class HrPayslip(models.Model):
    ...
    def l10n_mx_edi_get_xml_etree(self, cfdi=None):
        ...
        _logger.warning(tree.get('Total', objectify.fromstring(cfdi).get('total')))
        _logger.warning("=========================")
    ...
    def _l10n_mx_edi_create_cfdi(self):
        ...
        values['customer_rfc']=self.return_cust_rfc()
        

        # -Compute cfdi
        #cfdi = qweb.render(PAYSLIP_TEMPLATE, values=values)
        cfdi = self.env.ref('l10n_mx_edi_payslip.payroll12')._render(values)
    ...
    def return_cust_rfc(self):
        customer=False
        for i in self:
            customer=i.employee_id.user_id.partner_id
        if not customer:
            customer_rfc = False
